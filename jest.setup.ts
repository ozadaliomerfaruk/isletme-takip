...
jest.mock('@/lib/supabase', () => ({
  ...
  supabase: {
    // Finansal/yetki testleri bekledikleri DB yuzeyini suite icinde acikca
    // tanimlamali. Sessiz null-success, yanlis RPC veya tablo cagrisini yesile
    // cevirdigi icin varsayilan davranis bilerek fail-fast'tir.
    from: jest.fn((table: string) => {
      throw new Error(`JEST_SUPABASE_UNEXPECTED_FROM:${table}`);
    }),
    rpc: jest.fn((name: string) => {
      throw new Error(`JEST_SUPABASE_UNEXPECTED_RPC:${name}`);
    }),
    auth: {
