    ...
    if (activeRowId) {
      ...
      setRows(rows.map(r => {
        ...
        return {
          ...
          birimFiyat: urun?.alis_fiyati ? formatAmountForInput(urun.alis_fiyati) : r.birimFiyat,
        };
      }));
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (cariLinkEnabled && selectedCariId) {
        ...
        // kısmi başarıda hata fırlatır ama işlenen hareketler stoku ARTIRMIŞ kalır;
        // kullanıcı tekrar basınca AYNI satırları gönderip çift stok yazardı. allSettled
        ...
        results.forEach((r, i) => {
          if (r.status === 'fulfilled') {
            ...
            failedCount++;
            if (__DEV__) console.error('Toplu stok giriş — başarısız:', validRows[i].id, r.reason);
          }
        });
                        ...
                        <View style={styles.pickerItemMeta}>
                          ...
                          {urun.alis_fiyati > 0 && (
                            <Text style={styles.pickerItemPrice}>
                              {formatCurrency(urun.alis_fiyati, urun.currency)}
                            </Text>
                          )}
                          {urun.kdv_orani > 0 && (
