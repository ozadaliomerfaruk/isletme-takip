...
import { cariSatisSchema, type CariSatisFormData } from '@/lib/schemas/paymentForm';
...
export default function CariSatisPage() {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors } } = useForm<CariSatisFormData>({
    resolver: zodResolver(cariSatisSchema),
    defaultValues: {
      amount: '',
      description: '',
  ...
  const getErrorMessage = (field: keyof CariSatisFormData) => {
  ...
  const onSubmit = async (data: CariSatisFormData) => {
    try {
      if (data.isIleriTarihli) {
        ...
        if (data.reminderConfig.enabled && result?.id) {
          ...
          await scheduleTransactionReminder(
            ...
            `${t('transactions:types.cari_satis')}: ${formatCurrency(parseCurrency(data.amount))}${data.description ? ` - ${data.description}` : ''}`,
            reminderDate,
            {
        ...
        notifySaved(t('clients:messages.scheduledSaleCreated'));
        router.back();
      } else {
        ...
        notifySaved(t('clients:messages.saleRecorded'));
        router.back();
      }
    } catch (error) {
