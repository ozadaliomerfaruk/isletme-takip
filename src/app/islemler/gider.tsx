...
import { giderSchema, type GiderFormData } from '@/lib/schemas/paymentForm';
...
export default function GiderEklePage() {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors } } = useForm<GiderFormData>({
  ...
  const getErrorMessage = (field: keyof GiderFormData) => {
  ...
  const onSubmit = async (data: GiderFormData) => {
    try {
      if (data.isIleriTarihli) {
        ...
        if (data.reminderConfig.enabled && result?.id) {
          ...
          await scheduleTransactionReminder(
            ...
            `${t('transactions:types.gider')}: ${formatCurrency(parseCurrency(data.amount))}${data.description ? ` - ${data.description}` : ''}`,
            reminderDate,
            {
        ...
        Alert.alert(t('common:status.success'), t('transactions:messages.expenseAdded'), [
          { text: t('common:buttons.ok'), onPress: () => router.back() },
        ]);
      }
    } catch (error) {
        ...
        >
          <View style={styles.header}>
            <View style={styles.headerRow}>
              <Text variant="h2" style={styles.headerTitle}>{t('transactions:titles.addExpense')}</Text>
              <TouchableOpacity
