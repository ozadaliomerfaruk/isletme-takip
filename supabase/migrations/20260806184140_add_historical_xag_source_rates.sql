-- XAG kaynak para birimli islemleri bugunku kura dusurmeden tarihsel olarak
-- degerlemek icin gunluk tabloya gram gumus/TL serisi ekler. Mevcut mercekler
-- ve RPC imzalari korunur; yalniz kaynak para birimi cozumlemesine XAG eklenir.
--
-- Geriye uyumluluk: yeni kolon nullable'dir. Eski istemciler kolonu okumaz;
-- mevcut RPC imzalari ve JSON ciktilari degismez.

ALTER TABLE public.ekonomik_gostergeler_gunluk
  ADD COLUMN IF NOT EXISTS gram_gumus_try numeric(18, 8),
  ADD COLUMN IF NOT EXISTS gumus_yayim_zamani timestamp with time zone,
  ADD COLUMN IF NOT EXISTS gumus_source text;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'ekonomik_gostergeler_gunluk_silver_positive'
      AND conrelid = 'public.ekonomik_gostergeler_gunluk'::pg_catalog.regclass
  ) THEN
    ALTER TABLE public.ekonomik_gostergeler_gunluk
      ADD CONSTRAINT ekonomik_gostergeler_gunluk_silver_positive
      CHECK (gram_gumus_try IS NULL OR gram_gumus_try > 0);
  END IF;
END;
$constraint$;

COMMENT ON COLUMN public.ekonomik_gostergeler_gunluk.gram_gumus_try IS
  'MetalpriceAPI tarihsel XAG orta degerinden hesaplanan 1 gram gumusun TL karsiligi.';
COMMENT ON COLUMN public.ekonomik_gostergeler_gunluk.gumus_yayim_zamani IS
  'Tarihsel XAG gozleminin kaynak tarafindan erisilebilir oldugu zaman.';
COMMENT ON COLUMN public.ekonomik_gostergeler_gunluk.gumus_source IS
  'Gram gumus degerinin kaynagi; satir geneli source alanindan bagimsizdir.';

CREATE OR REPLACE FUNCTION public.get_category_report_lens_v1(
  p_isletme_id uuid,
  p_types text[],
...
BEGIN
  ...
  rate_keys AS MATERIALIZED (
    SELECT DISTINCT islem_gunu, referans_gunu, txn_currency
  ...
  rate_lookup AS MATERIALIZED (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE monthly.ay <= pg_catalog.date_trunc(
              'month', rate_key.referans_gunu::timestamp
            )::date
        AND monthly.ay >= (
          pg_catalog.date_trunc('month', rate_key.referans_gunu::timestamp)
          - '2 months'::interval
        )::date
...
TO authenticated;
-- Karşılaştırma ekranının bütün dönem kovalarını tek RPC'de hesaplar.
-- Nominal görünüm mevcut get_income_expense_summary fonksiyonunda kalır.
CREATE OR REPLACE FUNCTION public.get_income_expense_comparison_lens_v1(
  p_isletme_id uuid,
  p_start_dates timestamp with time zone[],
  p_end_dates timestamp with time zone[],
  p_lens text
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  v_reports_can_view boolean := false;
  v_can_see_all_users_data boolean := false;
  v_allowed_source_modules text[] := ARRAY[
    'hesaplar', 'cariler', 'urunler', 'personel'
  ]::text[];
  v_empty jsonb := pg_catalog.jsonb_build_object(
    'rows', '[]'::jsonb,
    'lens', p_lens,
...
BEGIN
  IF p_isletme_id IS NULL
     OR p_start_dates IS NULL
     OR p_end_dates IS NULL
     OR pg_catalog.cardinality(p_start_dates) < 1
     OR pg_catalog.cardinality(p_start_dates) > 40
     OR pg_catalog.cardinality(p_start_dates)
        <> pg_catalog.cardinality(p_end_dates)
     OR p_lens IS NULL
     OR NOT (p_lens = ANY (ARRAY['reel', 'usd', 'eur', 'altin']::text[]))
  THEN
    RETURN v_empty;
  ...
  IF EXISTS (
    SELECT 1
    FROM pg_catalog.generate_subscripts(p_start_dates, 1) AS bucket_index
    WHERE p_start_dates[bucket_index] IS NULL
       OR p_end_dates[bucket_index] IS NULL
       OR p_start_dates[bucket_index] > p_end_dates[bucket_index]
  ) THEN
    RETURN v_empty;
  END IF;
  ...
  WITH buckets AS MATERIALIZED (
    SELECT
      (bucket.ordinality - 1)::integer AS bucket_index,
      bucket.start_date,
      p_end_dates[bucket.ordinality] AS end_date
    FROM pg_catalog.unnest(p_start_dates) WITH ORDINALITY
      AS bucket(start_date, ordinality)
  ),
  eligible_islemler AS MATERIALIZED (
    SELECT
      ...
      CASE
        WHEN transaction_row.type = ANY (
          ARRAY['gelir', 'cari_satis', 'personel_satis']::text[]
        ) THEN 1
        WHEN transaction_row.type = 'cari_satis_iade' THEN -1
        ELSE 0
      END::numeric AS income_sign,
      CASE
        WHEN transaction_row.type = ANY (
          ARRAY['gider', 'cari_alis', 'personel_gider']::text[]
        ) THEN 1
        WHEN transaction_row.type = 'cari_alis_iade' THEN -1
        ELSE 0
      END::numeric AS expense_sign
    FROM buckets AS bucket
    INNER JOIN public.islemler AS transaction_row
      ON transaction_row.date >= bucket.start_date
     AND transaction_row.date <= bucket.end_date
    ...
    LEFT JOIN public.personel AS employee
      ON employee.id = transaction_row.personel_id
     AND employee.isletme_id = p_isletme_id
    WHERE transaction_row.type = ANY (
      ARRAY[
        'gelir', 'cari_satis', 'personel_satis', 'cari_satis_iade',
        'gider', 'cari_alis', 'personel_gider', 'cari_alis_iade'
      ]::text[]
    )
      AND internal.islem_tipi_modulu(transaction_row.type::text)
          <@ v_allowed_source_modules
      AND (
        v_can_see_all_users_data IS TRUE
        OR transaction_row.created_by = v_user_id
      ...
      AND (
        NOT EXISTS (
          ...
          WHERE movement.islem_id = transaction_row.id
            AND movement.isletme_id = p_isletme_id
        )
        OR EXISTS (
          SELECT 1
  ...
  rate_keys AS MATERIALIZED (
    SELECT DISTINCT referans_gunu, txn_currency
    FROM eligible_islemler
  ),
  latest_cpi AS (
    SELECT monthly.ay, monthly.tufe
  ...
  rated_islemler AS MATERIALIZED (
    ...
    FROM eligible_islemler AS eligible
    INNER JOIN rate_lookup AS rate
      ON rate.referans_gunu = eligible.referans_gunu
     AND rate.txn_currency = eligible.txn_currency
  ),
  bucket_rows AS (
    SELECT
      ...
      COALESCE(
        pg_catalog.sum(
          rated.amount * rated.conversion_factor * rated.income_sign
        ) FILTER (WHERE rated.rate_complete IS TRUE),
        0
      ) AS income,
      COALESCE(
        pg_catalog.sum(
          rated.amount * rated.conversion_factor * rated.expense_sign
        ) FILTER (WHERE rated.rate_complete IS TRUE),
        0
      ) AS expense
    FROM buckets AS bucket
    LEFT JOIN rated_islemler AS rated
      ON rated.bucket_index = bucket.bucket_index
    GROUP BY bucket.bucket_index
  ),
  meta AS (
    SELECT pg_catalog.count(DISTINCT rated.id)
      FILTER (WHERE rated.rate_complete IS NOT TRUE) AS missing_rate_count
    FROM rated_islemler AS rated
  )
  SELECT pg_catalog.jsonb_build_object(
      ...
      (
        SELECT pg_catalog.jsonb_agg(
          pg_catalog.jsonb_build_object(
            'bucket_index', bucket_row.bucket_index,
            'income', bucket_row.income,
            'expense', bucket_row.expense
          )
          ORDER BY bucket_row.bucket_index
        )
        FROM bucket_rows AS bucket_row
      ),
      '[]'::jsonb
    ),
    'lens', p_lens,
...
ALTER FUNCTION public.get_income_expense_comparison_lens_v1(
  uuid, timestamp with time zone[], timestamp with time zone[], text
) OWNER TO postgres;
REVOKE ALL
ON FUNCTION public.get_income_expense_comparison_lens_v1(
  uuid, timestamp with time zone[], timestamp with time zone[], text
)
FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE
